# scripts/30_benchmark.sh
#!/usr/bin/env bash
set -euo pipefail

# Config
: "${BASE:=http://kubefix.kubefix.svc.cluster.local:8080}"
: "${TRIALS:=10}"

bench_pod="bench"
bench_ns="kubefix"
model="${GEMINI_MODEL:-gemini-2.0-flash}"

echo "Using in-cluster URL: $BASE"
echo "Bench pod: ${bench_ns}/${bench_pod}"

# Ensure bench pod exists and is ready
kubectl -n "$bench_ns" get pod "$bench_pod" >/dev/null 2>&1 || {
  echo "ERROR: bench pod not found. Create it first:"
  echo "kubectl -n kubefix run bench --image=curlimages/curl:8.11.1 --restart=Never -- sh -lc 'sleep 36000'"
  exit 1
}
kubectl -n "$bench_ns" wait --for=condition=Ready pod/"$bench_pod" --timeout=60s >/dev/null

# Prepare results file
out_csv="results.csv"
if ! [ -f "$out_csv" ]; then
  echo "scenario,model,elapsed_sec,rbac_patch,netpol_patch,podsec_patch,confidence,status" > "$out_csv"
fi

call_generate () {
  local ns="$1" pod="$2" scenario="$3"
  local payload
  payload=$(printf '{"mic":{"kind":"Pod","namespace":"%s","name":"%s"}}' "$ns" "$pod")
  local t0 t1 elapsed
  t0=$(date +%s)

  # Call from inside cluster via bench pod
  local resp
  set +e
  resp=$(kubectl -n "$bench_ns" exec "$bench_pod" -- sh -lc \
    "printf '%s' '$payload' | curl -s -X POST '$BASE/generate' -H 'Content-Type: application/json' --data-binary @-")
  rc=$?
  set -e
  t1=$(date +%s)
  elapsed=$((t1 - t0))

  if [ $rc -ne 0 ] || [ -z "$resp" ]; then
    echo "$scenario,$model,$elapsed,false,false,false,0,ERROR" | tee -a "$out_csv"
    return 1
  fi

  # Parse with jq inside your shell (host)
  local rbac netpol podsec conf status
  rbac=$(echo "$resp" | jq -r '.gen.rbac_patch   | (.|type=="string")')
  netpol=$(echo "$resp" | jq -r '.gen.netpol_patch | (.|type=="string")')
  podsec=$(echo "$resp" | jq -r '.gen.podsec_patch | (.|type=="string")')
  conf=$(echo "$resp" | jq -r '.gen.confidence // 0')

  # Capture the out_dir so we can fetch patches afterwards
  local out_dir
  out_dir=$(echo "$resp" | jq -r '.out_dir')
  status="OK"

  echo "$scenario,$model,$elapsed,$rbac,$netpol,$podsec,$conf,$status" | tee -a "$out_csv"

  # Copy artifacts for this single run
  # (if you run inside-cluster only, you can also kubectl cp from the kubefix pod;
  # here we assume you'll do the bulk copy after the benchmark completes)
}

run_trials () {
  local scenario="$1" ns="$2" label="$3"
  echo "Running scenario=$scenario ns=$ns label=$label trials=$TRIALS"

  # Resolve a pod name each time (freshness)
  for i in $(seq 1 "$TRIALS"); do
    pod=$(kubectl -n "$ns" get pod -l "$label" -o jsonpath='{.items[0].metadata.name}')
    if [ -z "${pod:-}" ]; then
      echo "$scenario,$model,0,false,false,false,0,NO_POD" | tee -a "$out_csv"
      continue
    fi
    call_generate "$ns" "$pod" "$scenario" || true
    sleep 1
  done
}

# Scenarios
run_trials "RBAC_PODSEC"  "demo"         "app=nginx"
run_trials "NETPOL"       "demo"         "app=nginx"
run_trials "PODSEC_ONLY"  "podsec-only"  "app=nginx"

echo "Benchmark complete → $out_csv"

